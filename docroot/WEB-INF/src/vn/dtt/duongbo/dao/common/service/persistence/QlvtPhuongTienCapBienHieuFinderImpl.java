...
public class QlvtPhuongTienCapBienHieuFinderImpl extends BasePersistenceImpl<QlvtPhuongTienCapPhuHieu> implements QlvtPhuongTienCapBienHieuFinder {
	
	private Log _log = LogFactoryUtil.getLog(QlvtPhuongTienCapBienHieuFinderImpl.class);
	...
	public List<Long> findIdNoiDungHoSoId(long noiDungHoSoId)
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				query.append("SELECT ID as id FROM qlvt_phuongtiencapbienhieu WHERE ");
	...
	public boolean deleteAllByNoiDungHoSoId(long noiDungHoSoId) throws SystemException {
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				StringBuilder query = new StringBuilder();
				query.append("delete from qlvt_phuongtiencapbienhieu where NOIDUNGHOSOID = ?");
				
				_log.info("===deleteAllByNoiDungHoSoId===" + query.toString());
