...
public class QlvtPhuongTienCapPhuHieuFinderImpl extends BasePersistenceImpl<QlvtPhuongTienCapPhuHieu> implements QlvtPhuongTienCapPhuHieuFinder {
	
	private Log _log = LogFactoryUtil.getLog(QlvtPhuongTienCapPhuHieuFinderImpl.class);
	...
	public List<Long> findIdNoiDungHoSoId(long noiDungHoSoId)
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				query.append("SELECT ID as id FROM qlvt_phuongtiencapphuhieu WHERE ");
	...
	public boolean deleteAllByNoiDungHoSoId(long noiDungHoSoId) throws SystemException {
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				StringBuilder query = new StringBuilder();
				query.append("delete from qlvt_phuongtiencapphuhieu where NOIDUNGHOSOID = ?");
