...
public class QlvtPhuongTienDangKyVietTrungFinderImpl extends BasePersistenceImpl<QlvtPhuongTienDangKyVietTrung> implements QlvtPhuongTienDangKyVietTrungFinder {
	
	private Log _log = LogFactoryUtil.getLog(QlvtPhuongTienDangKyVietTrungFinderImpl.class);
	...
	public List<Long> findIdNoiDungHoSoId(long noiDungHoSoId)
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				query.append("SELECT ID as id FROM qlvt_phuongtiendangky_vt WHERE ");
	...
	public boolean deleteAllByNoiDungHoSoId(long noiDungHoSoId) throws SystemException {
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				StringBuilder query = new StringBuilder();
				query.append("delete from qlvt_phuongtiendangky_vt where NOIDUNGHOSOID = ?");
