...
public class QlvtLoaiHinhHoatDongDnFinderImpl extends BasePersistenceImpl<QlvtLoaiHinhHoatDongDn> implements QlvtLoaiHinhHoatDongDnFinder {

private Log _log = LogFactoryUtil.getLog(QlvtLoaiHinhHoatDongDnFinderImpl.class);
	...
	public List<Long> findIdNoiDungHoSoId(long noiDungHoSoId) throws SystemException {
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				query.append("SELECT ID as id FROM qlvt_loaihinhhoatdong WHERE ");
	...
	public boolean deleteAllByNoiDungHoSoId(long noiDungHoSoId) throws SystemException {
		...
		try {
			if (noiDungHoSoId > 0) {
				...
				StringBuilder query = new StringBuilder();
				query.append("delete from qlvt_loaihinhhoatdong where NOIDUNGHOSOID = ?");
