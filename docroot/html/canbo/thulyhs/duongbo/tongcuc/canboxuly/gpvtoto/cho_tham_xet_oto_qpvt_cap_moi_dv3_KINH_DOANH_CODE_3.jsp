...
System.out.println("=============="+col1 + col2 + xuLyPhuHieuId);
...
<div class="modal-header">
	<h4 class="modal-title"><%=lstldkd.get(2).getName()%></h4>
</div>
<aui:row>
	
	<aui:col width="50">
		<aui:field-wrapper label="màu sơn:" inlineField="<%=true %>" >
				<select id="<%=QLVTNoiDiaTerm.MAU_SON%>"
						name="<%=QLVTNoiDiaTerm.MAU_SON%>%>"
						class="form-control">
						<%
	...
	<aui:col width="50">
		<aui:field-wrapper label="Thiết bị giám sát hành trình:" inlineField="<%=true %>" >
			<aui:select name="<%=QLVTNoiDiaTerm.THIET_BI_GIAM_SAT_HANH_TRINH %>" label=" ">
				<%
...
<div id="divKoDat"style="display: none;margin-left: 20%;">
	<aui:input type="textarea" name="<%=QLVTNoiDiaTerm.LY_DO_KHONG_DAT %>" cssClass="myTextArea"></aui:input>
</div>
