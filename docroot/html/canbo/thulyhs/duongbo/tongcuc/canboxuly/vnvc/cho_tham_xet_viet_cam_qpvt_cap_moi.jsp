	...
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Cơ quan cấp: " inlineField="<%=true %>" >
				<aui:input type="text" name="" value="<%=ettThongTinHoSo.getCoQuanCapDkkd() %>" label="">
...
AUI().use(
  ...
  function(A) {
    new A.DatePicker(
      {
        trigger: '#<portlet:namespace /><%=QLVTQuocTeTerm.NGAY_CAP_GIAY_PHEP_KDVT_LAN_DAU %>',
        mask: '%m/%d/%Y',
        popover: {
          zIndex: 1
        },
...
AUI().one('#<portlet:namespace /><%=QLVTQuocTeTerm.NGAY_CAP_GIAY_PHEP_KDVT_LAN_DAU%>').val('<%=Validator.isNotNull(ettThongTinHoSo.getNgayCapDkkd())?sdf.format(ettThongTinHoSo.getNgayCapDkkd()):StringPool.BLANK %>');
