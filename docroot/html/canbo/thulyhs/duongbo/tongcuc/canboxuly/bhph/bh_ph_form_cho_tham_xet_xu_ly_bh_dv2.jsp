...
<%
	...
	if(Validator.isNotNull(popUp)){
		...
		showMyForm.setParameter("mvcPath", "/html/canbo/thulyhs/duongbo/tongcuc/canboxuly/bhph/bh_ph_form_cho_tham_xet_xu_ly_bh_dv3.jsp");
	}
	showMyForm.setParameter("xuLyPhuHieuId",xuLyPhuHieuId+"");
	List<QLVTXuLyBienHieuPhuongTien> listBase = QLVTXuLyBienHieuPhuongTienLocalServiceUtil.findBycapPhuHieuID(xuLyPhuHieuId);
%>
...
int ipk = 0;
for(QLVTXuLyBienHieuPhuongTien ett: listBase){
	resultsQLVTListDV2.add(ipk, new QLVTListDV2(ett.getId()+"",ett.getBienSo(),ett.getKetQuaXuLy(),ett.getHinhThucSoHuuXe(),ett.getLinhVucKinhDoanhXeDuLich(),StringPool.BLANK));
	ipk++;
}
