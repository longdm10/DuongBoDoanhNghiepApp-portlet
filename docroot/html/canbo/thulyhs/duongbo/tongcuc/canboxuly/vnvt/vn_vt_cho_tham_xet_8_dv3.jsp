...
<aui:row>
	<aui:col width="100">
		<aui:field-wrapper label="Điểm dừng nghỉ trên đường*:" inlineField="<%=true %>" >
			<aui:input type="text" name="<%=QlvtLienVanTerm.DIEM_DUNG_NGHI %>" value="" label="">
...
<aui:row>
	...
	<aui:col width="50">
		<aui:field-wrapper label="Hình thức hoạt động *:" inlineField="<%=true %>" >
			<aui:select cssClass="fixWidth" id="<%=QlvtLienVanTerm.HINH_THUC_HOAT_DONG%>" name="<%=qlvtXuLyGpLienVanPhuongTien.getHinhThucHoatDong()%>" label="">
...
<div id="divDat" style="display: none;margin-left: 20%;">
	...
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Từ:" inlineField="<%=true %>" >
				<aui:input prefix="date" type="text" name="<%= QlvtLienVanTerm.XIN_CAP_PHEP_TU_NGAY %>" value="<%=qlvtXuLyGpLienVanPhuongTien.getXinCapPhepTuNgay()%>" label="">
		...
		<aui:col width="50">
			<aui:field-wrapper label="Đến:" inlineField="<%=true %>" >
				<aui:input prefix="date" type="text" name="<%=QlvtLienVanTerm.XIN_CAP_PHEP_DEN_NGAY%>" value="<%=qlvtXuLyGpLienVanPhuongTien.getXinCapPhepDenNgay()%>" label="">
	...
	<aui:row>
		<aui:col width="100">
			<aui:field-wrapper label="Hành trình: " inlineField="<%=true %>" >
				<aui:input type="textarea" name="<%=QlvtLienVanTerm.HANH_TRINH %>" value="" label="">
...
var A = AUI();
	...
	AUI().use(
			  ...
			  function(A) {
			    new A.DatePicker(
			      {
			    	trigger: '#<portlet:namespace /><%=QlvtLienVanTerm.XIN_CAP_PHEP_DEN_NGAY %>',
			        mask: '%m/%d/%Y',
			        popover: {
			          zIndex: 1
			        },
