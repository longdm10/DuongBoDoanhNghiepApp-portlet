...
PortletURL bienHieu_oTo_HTHS = renderResponse.createActionURL();
bienHieu_oTo_HTHS.setParameter(ActionRequest.ACTION_NAME, "bienHieu_oTo_HTHS");
bienHieu_oTo_HTHS.setParameter(Constants.ID_HO_SO_THU_TUC, hoSoThuTucId);
bienHieu_oTo_HTHS.setParameter(Constants.ID_PHIEU_XU_LY, phieuXuLyId);
bienHieu_oTo_HTHS.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
bienHieu_oTo_HTHS.setParameter(Constants.ID_QUY_TRINH_THU_TUC, quyTrinhThuTucId);
bienHieu_oTo_HTHS.setParameter(Constants.ID_THU_TUC_HANH_CHINH, idThuTucHanhChinh);
...
<%
	...
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người trả kết quả:" inlineField="<%=true %>" >
				<aui:input type="text"  name="<%=QLVTBienHieuTerm.NGUOI_LIEN_HE_TRA_KET_QUA %>" value="<%=thongTinHoSo.getNguoiLienHeTraKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTBienHieuTerm.SO_DIEN_THOAI_NGUOI_LIEN_HE %>" value="<%=thongTinHoSo.getSoDienThoaiNguoiLienHe()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người nhận kết quả:" inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTBienHieuTerm.NGUOI_NHAN_KET_QUA %>" value="<%=thongTinHoSo.getNguoiNhanKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người nhận kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTBienHieuTerm.THONG_TIN_TRA_KET_QUA %>" value="<%=thongTinHoSo.getThongTinTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:col width="50" >
			<aui:field-wrapper label="ngày trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTBienHieuTerm.NGAY_TRA_KET_QUA %>" value="<%=thongTinHoSo.getNgayTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
	</aui:col>
	...
	<div style = "margin-top: 20px;">
		...
		<aui:input type="hidden" name="actionURL" value="<%=bienHieu_oTo_HTHS.toString() %>"></aui:input>
