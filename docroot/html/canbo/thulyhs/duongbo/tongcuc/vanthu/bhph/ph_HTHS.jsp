...
PortletURL phuHieu_oTo_HTHS = renderResponse.createActionURL();
phuHieu_oTo_HTHS.setParameter(ActionRequest.ACTION_NAME, "phuHieu_oTo_HTHS");
phuHieu_oTo_HTHS.setParameter(Constants.ID_HO_SO_THU_TUC, hoSoThuTucId);
phuHieu_oTo_HTHS.setParameter(Constants.ID_PHIEU_XU_LY, phieuXuLyId);
phuHieu_oTo_HTHS.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
phuHieu_oTo_HTHS.setParameter(Constants.ID_QUY_TRINH_THU_TUC, quyTrinhThuTucId);
phuHieu_oTo_HTHS.setParameter(Constants.ID_THU_TUC_HANH_CHINH, idThuTucHanhChinh);
...
<%
	...
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người trả kết quả:" inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTPhuHieuTerm.NGUOI_LIEN_HE_TRA_KET_QUA %>" value="<%=thongTinHoSo.getNguoiLienHeTraKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTPhuHieuTerm.SO_DIEN_THOAI_NGUOI_LIEN_HE %>" value="<%=thongTinHoSo.getSoDienThoaiNguoiLienHe()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người nhận kết quả:" inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTPhuHieuTerm.NGUOI_NHAN_KET_QUA %>" value="<%=thongTinHoSo.getNguoiNhanKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người nhận kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTPhuHieuTerm.THONG_TIN_TRA_KET_QUA %>" value="<%=thongTinHoSo.getThongTinTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:col width="50" >
			<aui:field-wrapper label="ngày trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTPhuHieuTerm.NGAY_TRA_KET_QUA %>" value="<%=thongTinHoSo.getNgayTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
	</aui:col>	
	...
	<div style = "margin-top: 20px;">
		...
		<aui:input type="hidden" name="actionURL" value="<%=phuHieu_oTo_HTHS.toString() %>"></aui:input>
...
AUI().use(
		  ...
		  function(A) {
		    new A.DatePicker(
		      {
		    	trigger: '#<portlet:namespace /><%=QLVTPhuHieuTerm.NGAY_TRA_KET_QUA %>',
		        mask: '%m/%d/%Y',
		        popover: {
		          zIndex: 1
		        },
...
<aui:script use="aui-io-request,aui-node">
	AUI().one('#<portlet:namespace /><%=QLVTPhuHieuTerm.NGAY_TRA_KET_QUA  %>').val('<%=Validator.isNotNull(thongTinHoSo.getNgayTraKetQua())?sdf.format(thongTinHoSo.getNgayTraKetQua()):StringPool.BLANK %>');
</aui:script>