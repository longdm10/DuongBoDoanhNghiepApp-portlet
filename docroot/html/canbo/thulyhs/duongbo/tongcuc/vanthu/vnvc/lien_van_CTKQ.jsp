...
PortletURL lienVan_vietCam_CTKQ = renderResponse.createActionURL();
lienVan_vietCam_CTKQ.setParameter(ActionRequest.ACTION_NAME, "lienVan_vietCam_CTKQ");
lienVan_vietCam_CTKQ.setParameter(Constants.ID_HO_SO_THU_TUC, hoSoThuTucId);
lienVan_vietCam_CTKQ.setParameter(Constants.ID_PHIEU_XU_LY, phieuXuLyId);
lienVan_vietCam_CTKQ.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
lienVan_vietCam_CTKQ.setParameter(Constants.ID_QUY_TRINH_THU_TUC, quyTrinhThuTucId);
lienVan_vietCam_CTKQ.setParameter(Constants.ID_THU_TUC_HANH_CHINH, idThuTucHanhChinh);
...
<aui:form name="fm" action="<%=lienVan_vietCam_CTKQ.toString()%>">
...
<aui:input type="hidden" name="keyDichChuyen"></aui:input>
	...
	<aui:input type="hidden" name="actionURL" value="<%=lienVan_vietCam_CTKQ.toString() %>"></aui:input>
...
//     A.one('#<portlet:namespace/>fm').removeAttribute('enctype').set('action',action);
      A.io(action, cfg);
      A.on('io:complete', function (id, o, args) {
    	  window.location.href ='<%=reTurnHomeURL.toString()%>';
      });
});
</aui:script>
