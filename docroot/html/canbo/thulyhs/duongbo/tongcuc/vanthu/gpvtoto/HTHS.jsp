...
PortletURL noiDia_oTo_HTHS = renderResponse.createActionURL();
noiDia_oTo_HTHS.setParameter(ActionRequest.ACTION_NAME, "noiDia_oTo_HTHS");
noiDia_oTo_HTHS.setParameter(Constants.ID_HO_SO_THU_TUC, hoSoThuTucId);
noiDia_oTo_HTHS.setParameter(Constants.ID_PHIEU_XU_LY, phieuXuLyId);
noiDia_oTo_HTHS.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
noiDia_oTo_HTHS.setParameter(Constants.ID_QUY_TRINH_THU_TUC, quyTrinhThuTucId);
noiDia_oTo_HTHS.setParameter(Constants.ID_THU_TUC_HANH_CHINH, idThuTucHanhChinh);
...
<div class="TabbedPanelsContent">
	...
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người trả kết quả:" inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTNoiDiaTerm.NGUOI_LIEN_HE_TRA_KET_QUA %>" value="<%=ettThongTinHoSo.getNguoiLienHeTraKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTNoiDiaTerm.SO_DIEN_THOAI_NGUOI_LIEN_HE %>" value="<%=ettThongTinHoSo.getSoDienThoaiNguoiLienHe()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:row>
		<aui:col width="50">
			<aui:field-wrapper label="Họ tên người nhận kết quả:" inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTNoiDiaTerm.NGUOI_NHAN_KET_QUA %>" value="<%=ettThongTinHoSo.getNguoiNhanKetQua() %>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
		<aui:col width="50">
			<aui:field-wrapper label="Điện thoại người nhận kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTNoiDiaTerm.THONG_TIN_TRA_KET_QUA %>" value="<%=ettThongTinHoSo.getThongTinTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
		</aui:col>
	</aui:row>
	<aui:col width="50" >
			<aui:field-wrapper label="ngày trả kết quả: " inlineField="<%=true %>" >
				<aui:input type="text" name="<%=QLVTNoiDiaTerm.NGAY_TRA_KET_QUA %>" value="<%=ettThongTinHoSo.getNgayTraKetQua()%>" label="">
				</aui:input> 
			</aui:field-wrapper>
	</aui:col>
	<br>
	<div style = "margin-top: 20px;">
		...
		<aui:input type="hidden" name="actionURL" value="<%=noiDia_oTo_HTHS.toString() %>"></aui:input>
...
AUI().use(
		  ...
		  function(A) {
		    new A.DatePicker(
		      {
		    	trigger: '#<portlet:namespace /><%=QLVTNoiDiaTerm.NGAY_TRA_KET_QUA %>',
		        mask: '%m/%d/%Y',
		        popover: {
		          zIndex: 1
		        },
...
<aui:script use="aui-io-request,aui-node">
	AUI().one('#<portlet:namespace /><%=QLVTNoiDiaTerm.NGAY_TRA_KET_QUA  %>').val('<%=Validator.isNotNull(ettThongTinHoSo.getNgayTraKetQua())?sdf.format(ettThongTinHoSo.getNgayTraKetQua()):StringPool.BLANK %>');
</aui:script>