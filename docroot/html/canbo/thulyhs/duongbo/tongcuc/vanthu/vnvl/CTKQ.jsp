...
PortletURL quocTe_vietLao_CTKQ = renderResponse.createActionURL();
quocTe_vietLao_CTKQ.setParameter(ActionRequest.ACTION_NAME, "quocTe_vietLao_CTKQ");
quocTe_vietLao_CTKQ.setParameter(Constants.ID_HO_SO_THU_TUC, hoSoThuTucId);
quocTe_vietLao_CTKQ.setParameter(Constants.ID_PHIEU_XU_LY, phieuXuLyId);
quocTe_vietLao_CTKQ.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
quocTe_vietLao_CTKQ.setParameter(Constants.ID_QUY_TRINH_THU_TUC, quyTrinhThuTucId);
quocTe_vietLao_CTKQ.setParameter(Constants.ID_THU_TUC_HANH_CHINH, idThuTucHanhChinh);
%>
<portlet:actionURL var="addItemURL" name="noiDia_oTo_CTKQ" />
<portlet:actionURL var="uploadURL" name="upLoadFileBieuMau" />
<aui:form name="fm" action="<%=quocTe_vietLao_CTKQ.toString()%>">
<div class="TabbedPanelsContent">
	...
	<aui:input type="hidden" name="actionURL" value="<%=quocTe_vietLao_CTKQ.toString() %>"></aui:input>
    ...
    function() {
        ...
        var errorBlock=A.one('#group-success-block');
         var cfg = {
          ...
          A.io('<%= uploadURL %>', cfg);<!-- submit form cfg -->
          
          <!-- json tra ve neu submit thanhcong -->
