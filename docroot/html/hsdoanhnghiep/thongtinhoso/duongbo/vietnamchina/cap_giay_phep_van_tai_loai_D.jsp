...
<%
	_log.info("/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamchina/cap_giay_phep_van_tai_loai_D.jsp");
	...
	DonXinCapGPVTLoaiDCuaPTTrungQuoc donLienVan = null;
	...
	if (Validator.isNotNull(noidungHoSo) && Validator.isNotNull(noidungHoSo.getNoiDungXml())) {
		...
		if (lstObj != null && lstObj.size() > 0) {
			donLienVan = (DonXinCapGPVTLoaiDCuaPTTrungQuoc) lstObj.get(0);
		}
	}
	
	if (donLienVan == null) { donLienVan = MauDonFactory.converXmlToObject(noiDungXmml, new DonXinCapGPVTLoaiDCuaPTTrungQuoc()); }
	...
	xemTruocNoiDung.setParameter(Constants.PATH_MAUDON_VIEW_PDF, "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamchina/cap_giay_phep_van_tai_loai_D_view.jsp");
%>
...
<!-- 					<b>Đề nghị cấp giấy phép</b><br> -->
			...
			<h3 class="tle text-center">ĐƠN XIN CẤP GIẤY PHÉP VẬN TẢI LOẠI D<br/>APPLICATION FORM FOR TRANSPORT PERMIT - D</h3>
			<div class="col-sm-12">
			<div class="text-center col-xs-12 MB20">
				...
				<div class="row M0">
					<div class="col-xs-4 form-group">
					    <label class="textlabel col-xs-12 PA0" for="sel1">Số (No) <span class="red">*</span> :</label><br>
					    <label class="textlabel col-xs-12 PA0" for="sel1">Do cơ quan của Việt Nam<br/>(Issued by Vietnamese authority) <span class="red">*</span> :</label><br>
					    <label class="textlabel col-xs-12 PA0" for="sel1">Cấp ngày (Date) <span class="red">*</span> :</label>
					</div>
					<div class="col-xs-8 form-group">
						<p><input type="text" name="soGiayPhepVCVN" id="soGiayPhepVCVN" class="form-control" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getSoGiayPhepVCVN())? "" : donLienVan.getSoGiayPhepVCVN())%>"></p>
						<p><input type="text" name="noiCapGiayPhepVCVN" id="noiCapGiayPhepVCVN" class="form-control" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getNoiCapGiayPhepVCVN())? "" : donLienVan.getNoiCapGiayPhepVCVN())%>"></p>
						<div class="input-group">
							<input type="text" id="ngayCapGiayPhepVCVN" name="ngayCapGiayPhepVCVN" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getNgayCapGiayPhepVCVN())? "" : donLienVan.getNgayCapGiayPhepVCVN())%>" class="form-control" onclick="gtCalendar('ngayCapGiayPhepVCVN')">
				...
				<label>6. Giấy phép (vận chuyển hàng quá khổ, quá tải hoặc hàng nguy hiểm)</label>
				<label class="col-xs-12">Permit (for carrying overload, overweight or dangerous goods)</label>
				<div class="row M0">
					<div class="col-xs-4 form-group">
					    <label class="textlabel col-xs-12 PA0" for="sel1">Số (No) <span class="red">*</span> :</label>
					    <label class="textlabel col-xs-12 PA0" for="sel1">Do cơ quan của Trung Quốc<br/>(Issued by Chinese authority) <span class="red">*</span> :</label>
					    <label class="textlabel col-xs-12 PA0" for="sel1">Cấp ngày (Date) <span class="red">*</span> :</label>
					</div>
					<div class="col-xs-8 form-group">
						<p><input type="text" name="soGiayPhepVCTQ" id="soGiayPhepVCTQ" class="form-control" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getSoGiayPhepVCTQ())? "" : donLienVan.getSoGiayPhepVCTQ())%>"></p>
						<p><input type="text" name="noiCapGiayPhepVCTQ" id="noiCapGiayPhepVCTQ" class="form-control" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getNoiCapGiayPhepVCTQ())? "" : donLienVan.getNoiCapGiayPhepVCTQ())%>"></p>
						<div class="input-group">
							<input type="text" id="ngayCapGiayPhepVCTQ" name="ngayCapGiayPhepVCTQ" value="<%=Validator.isNull(donLienVan)? "" : (Validator.isNull(donLienVan.getNgayCapGiayPhepVCTQ())? "" : donLienVan.getNgayCapGiayPhepVCTQ())%>" class="form-control" onclick="gtCalendar('ngayCapGiayPhepVCTQ')">
							<span class="input-group-addon" onclick="gtCalendar('ngayCapGiayPhepVCTQ')"><i class="fa fa-calendar" onclick="gtCalendar('ngayCapGiayPhepVCTQ')"></i></span>
						</div>
					</div>
				</div>
				<label>7. Nộp hồ sơ xin cấp giấy phép loại D cho các xe sau đây <span class="red">*</span></label>
				<label class="col-xs-12">Apply for Transport Permit - D for following vehicles</label>
				<div class="clear"></div>
				
				<!-- noi dung chuc nang Popup -->
				<%@ include file="/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamchina/table_phuong_tien_dang_ki.jsp"%>
				
				<label class="pull-left" style="margin-top:15px">8. Người liên hệ để trả kết quả cấp phép ( Contact person):</label>
				<div class="clear"></div>
				<div class="row M0">
					<div class="col-xs-12 form-group">
						<label class="col-xs-2 textlabel" for="sel1">Họ tên (Full name) <span class="red">*</span>:</label>
						<p class="col-xs-10">
					...
					<div class="col-xs-12 form-group">
						<label class="col-xs-2 textlabel" for="sel1">Số điện thoại (Tel) <span class="red">*</span>:</label>
						<p class="col-xs-10">
...
<script type="text/javascript">
	...
	function summitForm(url) {
		...
		var soGiayPhepVCTQ = document.getElementById("soGiayPhepVCTQ").value;
		var noiCapGiayPhepVCTQ = document.getElementById("noiCapGiayPhepVCTQ").value;
		var ngayCapGiayPhepVCTQ = document.getElementById("ngayCapGiayPhepVCTQ").value;
		...
		if (soGiayPhepVCVN == "") {
			alert("Số giấy phép vận chuyển không được để trống");
			return false;
		}
		if (noiCapGiayPhepVCVN == "") {
			alert("Nơi cấp giấy phép vận chuyển không được để trống");
			return false;
		}
		if (ngayCapGiayPhepVCVN == "") {
			alert("Ngày cấp giấy phép vận chuyển không được để trống");
			return false;
		}
		if(isDate(ngayCapGiayPhepVCVN) == false){
				alert("ngày cấp phép vận chuyển không đúng định dạng");
				return false;
			}
		if (soGiayPhepVCTQ == "") {
			alert("Số giấy phép vận chuyển Trung Quốc không được để trống");
			return false;
		}
		if (noiCapGiayPhepVCTQ == "") {
			alert("Nơi cấp giấy phép vận chuyển Trung Quốc không được để trống");
			return false;
		}
		if (ngayCapGiayPhepVCTQ == "") {
			alert("Ngày cấp giấy phép vận chuyển Trung Quốc không được để trống");
			return false;
		}
		if(isDate(ngayCapGiayPhepVCTQ) == false){
			alert("ngày cấp phép vận chuyển Trung Quốc không đúng định dạng");
			return false;
		}
		if (hoTenNguoiLienHeTrakq == "") {
			alert("Họ tên người liên hệ để trả kết quả không được để trống");
			return false;
		}
		if (sdtNguoiLienHeTrakq == "") {
			alert("Số điện thoại người liên hệ để trả kết quả không được để trống");
			return false;
		}
		if(isNormalInteger(sdtNguoiLienHeTrakq)){
			alert("Số điện thoại phải là dạng số!");
			return false;
		}
