...
<%
   ...
   // int randomNetCost = (netCost[new Random().nextInt(netCost.length)]);
    ...
    String randomNetCost = ParamUtil.getString(request, Constants.VPC_AMOUNT);
    ...
    String goodCode = "";
    String loaiHoaChat = "";
    List<Object> allObj = DoanhNghiepUtils.getXmlByHoSoThuTucId(hoSoThuTucId);
    ...
    goodCode = userId + "" +  (new SimpleDateFormat("yyyyMMddHHmm").format(calendar.getTime()));
    //goodCode = userId + "_" +  String.valueOf((new Date()).getTime());
    		
    String thongTinHoSo = "";
    thongTinHoSo =  thongTinHoSo + listNoiDungHoSo + "_" + thuTucHanhChinhId + "_" + hoSoThuTucId;
    ...
    if(randomNetCost == null || randomNetCost.length()==0 || randomNetCost.equals("0")){
    	randomNetCost="0";
    }
	...
	callUrl.setParameter(Constants.DANH_SACH_NOI_DUNG_HO_SO, listNoiDungHoSo);
		...
		<div class="main_chuanbihs">
			<form action="<%= callUrl.toString() %>" method="post" name="myForm">
				<div style="display: none;">>
					...
					<input type="hidden" name="net_cost" value="<%=randomNetCost%>"/>
