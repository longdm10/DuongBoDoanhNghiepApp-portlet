...
<%
	_log.info("/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thong_bao_dieu_chinh_tan_suat_tuyen_viet_cam.jsp");
	...
	TBaoDieuChinhTanSuatXeChayTuyenCDVietCam donLienVan =null;
	...
	if (hoSoThuTucId > 0) {
		...
		if (Validator.isNotNull(noidungHoSo) && Validator.isNotNull(noidungHoSo.getNoiDungXml())) {
			...
			_log.info("thong_bao_dieu_chinh_tan_suat_tuyen_viet_cam.jsp.jsp--noidungHoSo--" + noidungHoSo.getId());
			
			List<Object> lstObj = MauDonFactory.converXMLMessagesContentToObject(XmlUtils.replaceXML(noidungHoSo.getNoiDungXml()));
			
			if (lstObj != null && lstObj.size() > 0) {
				donLienVan = (TBaoDieuChinhTanSuatXeChayTuyenCDVietCam) lstObj.get(0);
			}
		}
	}
	
	if (donLienVan == null) { donLienVan = MauDonFactory.converXmlToObject(noiDungXmml, new TBaoDieuChinhTanSuatXeChayTuyenCDVietCam()); }
	if (donLienVan == null) { donLienVan = new TBaoDieuChinhTanSuatXeChayTuyenCDVietCam();}
	...
	List<DmDataItem> lstTuyenVietCam = DanhMucConstants.getTuyenVietCam();
%>
<%
	...
	xemTruocNoiDung.setParameter(Constants.PATH_MAUDON_VIEW_PDF, "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thong_bao_dieu_chinh_tan_suat_tuyen_viet_cam_view.jsp");
%>

<!------ /html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thong_bao_dieu_chinh_tan_suat_tuyen_viet_cam.jsp ---------->
		...
		<div class="BGT noidung">
			<div class="box1000" style="overflow: hidden;">
				...
				<h3 class="tle text-center">
					THÔNG BÁO TĂNG/GIẢM TẦN SUẤT CHẠY XE<br>TUYẾN VẬN TẢI HÀNH KHÁCH CỐ ĐỊNH<br>BẰNG XE Ô TÔ GIỮA VIỆT NAM VÀ CAMPUCHIA
				</h3>
				<%
				%>
				<div class="col-sm-12">
				...
				%>
					...
					<div style="padding: 0 15px; margin-bottom: 15px; overflow: hidden;">
						...
						<div class="col-xs-3 PA0">
							<input type="text" class="form-control" id="soGiayPhepVanTai" name="soGiayPhepVanTai" value="<%=(doanhNghiep.getSoGpkdVanTaiBangOto() == null) ? "" : doanhNghiep.getSoGpkdVanTaiBangOto()%>">
						</div>
						<div class="clear" style="margin-top: 10px"></div>
						<label class="pull-left w265">Ngày cấp <span class="red">*</span></label>
						<div class="col-xs-3 PA0">
							<div class="input-group">
								<input type="text" class="form-control" id="ngayCapGiayPhepVantai" name="ngayCapGiayPhepVantai" onclick="gtCalendar('ngayCapGiayPhepVantai')" value="<%=DateUtils.dateToString("dd/MM/yyyy", doanhNghiep.getNgayCapGpkdVanTaiBangOto())%>" >
					...
					<div style="margin-left: 280px; clear: both;">
						<p style="text-align: left; margin-top: 10px !important;">
							<span style="font-family: Arial">doanh nghiệp (HTX) Công ty <%=doanhNghiep.getTenDoanhNghiep().toUpperCase()%> sẽ tăng/giảm tần suất
					...
					<div class="MB10" style="margin-left: 280px; width: 63%">
						<select class="form-control" id="tuyen" name="tuyen">
							...
							<%
								for (DmDataItem item : lstTuyenVietCam) {
							%>
