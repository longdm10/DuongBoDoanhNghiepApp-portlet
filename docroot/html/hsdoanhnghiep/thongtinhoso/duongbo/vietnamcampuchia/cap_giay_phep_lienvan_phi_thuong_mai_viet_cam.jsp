<%@page import="vn.dtt.duongbo.maudon.model.DonXinCapGPLienVanVietCam"%>
...
<%
	_log.info("/html/hsdoanhnghiep/thongtinhoso/vietnamcampuchia/cap_giay_phep_lienvan_phi_thuong_mai_viet_cam.jsp");
	...
	//DM_LHHD - 6. Loại hình kinh doanh vận tải
	//cam / lao
	...
	DonXinCapGPLienVanVietCam donLienVan = null;
	...
	if (Validator.isNotNull(noidungHoSo) && Validator.isNotNull(noidungHoSo.getNoiDungXml())) {
		...
		if (lstObj != null && lstObj.size() > 0) {
			donLienVan = (DonXinCapGPLienVanVietCam) lstObj.get(0);
		}
	}
	
	if (donLienVan == null) { donLienVan = MauDonFactory.converXmlToObject(noiDungXmml, new DonXinCapGPLienVanVietCam()); }
%>
<%
...
xemTruocNoiDung.setParameter(Constants.PATH_MAUDON_VIEW_PDF, "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/cap_giay_phep_lienvan_phi_thuong_mai_viet_cam_view.jsp");
%>
	...
	<div class="BGT noidung">
		<div class="box1000">
			...
			<%} else if (Constants.LVTT_VIET_NAM_CAMPUCHIA.equalsIgnoreCase(loaiTthc)) {%>
				<h3 class="tle text-center">ĐƠN ĐỀ NGHỊ<br>CẤP GIẤY PHÉP LIÊN VẬN VIỆT NAM - CAMPUCHIA</h3>
			<%}%>
			...
			<div class="text-center col-xs-12 MB20">
				...
				<label class="pull-left">5. Đề nghị cấp Tổng cục Đường bộ Việt Nam (hoặc Sở Giao thông vận tải địa phương) cấp Giấy phép liên vận Việt - Campuchia cho phương tiện vận tải sau <span class="red">*</span>:</label>
				...
				<div class="row M0">
					...
					List<String> lstResultMucDichChuyenDi = Validator.isNotNull(donLienVan) ? donLienVan.getMucDichChuyenDi() : new ArrayList<String>();
				...
				<%
				if(Constants.TTHC_VIET_CAM_LIENVAN_PHUONGTIEN_PHITHUONGMAI_MAT_MAT.equals(thuTucHanhChinh.getMaThuTuc().trim())){
					%>
					<div class="clear"></div>
