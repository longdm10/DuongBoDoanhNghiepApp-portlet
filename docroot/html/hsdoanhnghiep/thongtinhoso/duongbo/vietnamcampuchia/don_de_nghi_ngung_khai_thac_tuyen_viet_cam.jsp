...
<%@page import="vn.dtt.duongbo.maudon.model.NgungKhaiThacTuyenVTHKCDBangOtoVietCam"%>
...
<%
	_log.info("/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/don_de_nghi_ngung_khai_thac_tuyen_viet_cam.jsp");
	...
	NgungKhaiThacTuyenVTHKCDBangOtoVietCam donLienVan = null;
	...
	if (Validator.isNotNull(noidungHoSo) && Validator.isNotNull(noidungHoSo.getNoiDungXml())) {
		...
		_log.info("--noidungHoSo--" + noidungHoSo.getNoiDungXml());
	
		List<Object> lstObj = MauDonFactory.converXMLMessagesContentToObject(noidungHoSo.getNoiDungXml());
		if (lstObj != null && lstObj.size() > 0) {
			donLienVan = (NgungKhaiThacTuyenVTHKCDBangOtoVietCam) lstObj.get(0);
		}
	}
	
	if (donLienVan == null) { donLienVan = MauDonFactory.converXmlToObject(noiDungXmml, new NgungKhaiThacTuyenVTHKCDBangOtoVietCam()); }
...
xemTruocNoiDung.setParameter(Constants.PATH_MAUDON_VIEW_PDF, "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/don_de_nghi_ngung_khai_thac_tuyen_viet_cam_view.jsp");
%>
...
<!-- 							<option value="">-- Lựa chọn Tỉnh --</option> -->
				...
				<h3 class="tle text-center">
					THÔNG BÁO<br>NGỪNG KHAI THÁC TUYẾN VẬN TẢI HÀNH KHÁCH<br>CỐ ĐỊNH BẰNG XE Ô TÔ GIỮA VIỆT NAM - CAMPUCHIA
				</h3>
				...
				<div class="col-sm-12">
					...
					<div class="text-center col-xs-12 MB20">
						...
						<div class="col-xs-5">
							<select class="form-control" id="coQuanGui" name="coQuanGui">
								<option>-- Chọn cơ quan cấp phép --</option>
								<%
					...
					<div class="input-group" style="width: 25%">
						<input type="text" class="form-control" id="ngayNgungKhaiThac" name="ngayNgungKhaiThac" onclick="gtCalendar('ngayNgungKhaiThac')" value="<%=donLienVan!=null?donLienVan.getNgayNgung():""%>"> <span
							class="input-group-addon" onclick="gtCalendar('ngayNgungKhaiThac')"><i class="fa fa-calendar"></i></span>
					</div>
