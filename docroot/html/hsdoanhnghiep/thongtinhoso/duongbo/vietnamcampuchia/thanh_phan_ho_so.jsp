...
thanhPhanHoSoUrl.setParameter("jspPage", "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thanh_phan_ho_so.jsp");
...
System.out.println("---------------/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thanh_phan_ho_so.jsp------------");
				...
				<div class="modal-content">
					...
					<form name="<%=popupFormName %>" id="<%=popupFormName%>" action="<%=actionUploadFileBieuMau %>" method="POST" enctype="multipart/form-data">
						...
						<div class="modal-footer">
							<div class="col-xs-12">
								<input class="btn btn-primary" type="button" value="Tải lên" onclick="summitPopupForm('<%=actionUploadFileBieuMau%>', '<%=thanhPhanHoSo.getId()%>', '<%=allBieuMauHoSo.size() %>')" />
							</div>
						</div>
					</form>
				</div>
...
%>
<liferay-ui:success key="<%=Constants.THAO_TAC_THANH_CONG %>" message="Yêu cầu của bạn được thực hiện thành công."></liferay-ui:success>
<liferay-ui:error key="<%=Constants.THAO_TAC_THAT_BAI %>" message="Yêu cầu của bạn được thực hiện không thành công."></liferay-ui:error>
...
<div class="P10 col-xs-12">
	<div class="BGT noidung">
				...
				<tbody>
					...
					if (allThanhPhanHoSo != null && allThanhPhanHoSo.size() > 0) {
						...
						for (int i = 0; i < allThanhPhanHoSo.size(); i++) {
							...
							nhapDuLieuTrucTuyen.setParameter(Constants.LOAI_THU_TUC_HANH_CHINH, Constants.LVTT_VIET_NAM_CAMPUCHIA);
							%>
							<tr>
					...
					if (allThanhPhanHoSoCanBoTraVe != null && allThanhPhanHoSoCanBoTraVe.size() > 0) {
						String srcImage = request.getContextPath() + "/img/i12.png";
						String srcImageThaoTacEditUpload = "";
						String srcImageThaoTacEditUploadUpdate = "";
						String srcImageThaoTacEditUploadReplace = "";
						...
						for (int i = 0; i < allThanhPhanHoSoCanBoTraVe.size(); i++) {
							...
							allNoiDungHoSo = TthcNoidungHoSoLocalServiceUtil.findByF_HS_DINH_KEM(hoSoThuTucId);
							
							String tagLinkString = "";
							if(Validator.isNotNull(allNoiDungHoSo) && allNoiDungHoSo.size() > 0){
							%>
							<tr>
								<td class="text-center"><%=i + 1%></td>
								<td class="text-center"><a><img src="<%=srcImage %>"></a></td>
									...
									<td class="sp" style="word-wrap: break-word;">
										...
										if (allNoiDungHoSo != null && allNoiDungHoSo.size() > 0) {
											...
											PortletURL xemNoidungFileDinhKem = renderResponse.createRenderURL();
											xemNoidungFileDinhKem.setParameter("jspPage", "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/view_thong_tin_mau_don.jsp");
											xemNoidungFileDinhKem.setParameter("backUrl", "/html/hsdoanhnghiep/thongtinhoso/duongbo/vietnamcampuchia/thanh_phan_ho_so_view.jsp");
											xemNoidungFileDinhKem.setParameter(Constants.ID_THU_TUC_HANH_CHINH, String.valueOf(thuTucHanhChinhId));
											xemNoidungFileDinhKem.setParameter(Constants.ID_HO_SO_THU_TUC, String.valueOf(hoSoThuTucId));
											xemNoidungFileDinhKem.setParameter(Constants.ID_THANH_PHAN_HO_SO, String.valueOf(thanhPhanHoSo.getId()));
											xemNoidungFileDinhKem.setParameter("listNoiDungHoSo", listNoiDungHoSo);
		...
		<div class="col-xs-12">
			...
			%>
				<c:choose>
					<c:when test="<%=Validator.isNotNull(hoSoThuTuc)? hoSoThuTuc.getTrangThaiHoSo().equals(\"DN_CBS\"):false %>">
						<button class="btn btn-primary" type="button" onclick="summitForm('<%=actionNopHoSoBoSung%>')">Nộp hồ sơ bổ sung</button>
					</c:when>
					<c:otherwise>
...
<liferay-ui:success key="<%=PortalUtil.getPortletId(request) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_SUCCESS_MESSAGE%>" message="duysgdyuagsyugasuydguy gyuasgdyu" />
