...
<%
DonDTTPHS192 donOnline = (DonDTTPHS192) DuongThuyUtil.getDonOnline(noiDungHoSoId, noiDungXML);

if(donOnline == null) {
	donOnline = new DonDTTPHS192();
}
...
mapDonValue.put(DonOnline.DIA_CHI_CAP_LAI_MOI, GetterUtil.getString(donOnline.getDiaChiCapLaiMoi()));
mapDonValue.put(DonOnline.LY_DO_THAY_DOI_DIA_CHI, GetterUtil.getString(donOnline.getLyDoThayDoiDiaChi()));
...
<aui:form action="<%= actionXemTruocNoiDung %>" name="fm" method="post" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveDon();" %>'>
	<div class="P10 col-xs-12">
		<div class="BGT noidung">
			<div class="col-xs-12">
				<h3 class="tle text-center">ĐƠN ĐỀ NGHỊ ĐĂNG KÝ<br/>PHƯƠNG TIỆN THỦY NỘI ĐỊA<br/><span style="font-size:16px"><i>(Dùng cho phương tiện chỉ thay đổi cơ quan đăng ký phương tiện)</i></span></h3>
			</div>
			<div class="col-xs-12">
				...
				<label class="textlabel col-xs-5" >Nay đề nghị cơ quan đăng ký lại phương tiện theo địa chỉ mới là: <span class="red">*</span></label>
				<p class="col-xs-7">
					<%= DuongThuyUtil.renderInput(renderResponse.getNamespace(), 
							DonOnline.DIA_CHI_CAP_LAI_MOI, "form-control ", mapDonValue, mapDonRequired) %>
				</p>
				<label class="textlabel col-xs-5" >Lý do thay đổi địa chỉ: <span class="red">*</span></label>
				<p class="col-xs-7">
					<%= DuongThuyUtil.renderInput(renderResponse.getNamespace(), 
							DonOnline.LY_DO_THAY_DOI_DIA_CHI, "form-control ", mapDonValue, mapDonRequired) %>
				</p>
				<div class="clear"></div>
