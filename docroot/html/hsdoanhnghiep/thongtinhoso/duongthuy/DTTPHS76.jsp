...
<%
DonDTTPHS76 donOnline = (DonDTTPHS76) DuongThuyUtil.getDonOnline(noiDungHoSoId, noiDungXML);

List<Map<String, String>> listQuaTrinhLamViec = new ArrayList<Map<String, String>>();

if(donOnline == null) {
	donOnline = new DonDTTPHS76();
}

// Khoi tao List QuaTrinhLamViec cho don
if(donOnline.getQuaTrinhLamViecPTTND() == null || donOnline.getQuaTrinhLamViecPTTND().size() == 0) {
	donOnline.getQuaTrinhLamViecPTTND().add(new DonDTTPHS76.QuaTrinhLamViecPTTND());
}

for(int i = 0; i < donOnline.getQuaTrinhLamViecPTTND().size(); i++) {
	DonDTTPHS76.QuaTrinhLamViecPTTND quaTrinhLamViec = donOnline.getQuaTrinhLamViecPTTND().get(i);
...
List<DmDataItem> listDieuKien = DmDataItemLocalServiceUtil.findByGroupCode(DTConstants.DANH_MUC.DM_GCNKNCM_CCCM);
...
<aui:form action="<%= actionXemTruocNoiDung %>" name="fm" method="post" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveDon();" %>'>
	<div class="P10 col-xs-12">
		<div class="BGT noidung">
			<div class="col-xs-12">
				<h3 class="tle text-center">ĐƠN ĐỀ NGHỊ<br/>CẤP, CẤP LẠI, CHUYỂN ĐỔI GCNKNCM, CCCM</h3>
			</div>
			<div class="col-xs-12">
				
				<%@ include file="/html/hsdoanhnghiep/thongtinhoso/duongthuy/ttc_don_1.jspf"%>
