...
<%
DonDTTPHS147 donOnline = (DonDTTPHS147) DuongThuyUtil.getDonOnline(noiDungHoSoId, noiDungXML);

if(donOnline == null) {
	donOnline = new DonDTTPHS147();
}
...
mapDonValue.put(DonOnline.CHIEU_RONG_THIET_KE, GetterUtil.getString(donOnline.getChieuRongThietKe()));
...
<aui:form action="<%= actionXemTruocNoiDung %>" name="fm" method="post" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveDon();" %>'>
	<div class="P10 col-xs-12">
		<div class="BGT noidung">
			<div class="col-xs-12">
				<h3 class="tle text-center">ĐƠN ĐỀ NGHỊ ĐĂNG KÝ<br/>PHƯƠNG TIỆN THỦY NỘI ĐỊA<br/><span style="font-size:16px"><i>(Dùng cho phương tiện chuyển từ cơ quan đăng ký khác sang cơ quan đăng ký phương tiện thủy nội địa)</i></span></h3>
			</div>
			<div class="col-xs-12">
				...
				<div class="col-xs-12 PA0">
					<label class="textlabel col-xs-2" for="sel1">Tên cơ quan đăng ký trước đây: <span class="red">*</span></label>
					<p class="col-xs-10">
						<%= DuongThuyUtil.renderInput(renderResponse.getNamespace(), 
							DonOnline.TEN_CO_QUAN_DANG_KY_TRUOC_DAY, "form-control", mapDonValue, mapDonRequired) %>
