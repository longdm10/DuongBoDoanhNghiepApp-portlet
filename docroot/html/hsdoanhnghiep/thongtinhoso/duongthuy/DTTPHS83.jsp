...
DonDTTPHS83 donOnline = (DonDTTPHS83) DuongThuyUtil.getDonOnline(noiDungHoSoId, noiDungXML);

List<Map<String, String>> listQuaTrinhLamViec = new ArrayList<Map<String, String>>();

if(donOnline == null) {
	donOnline = new DonDTTPHS83();
}

// Khoi tao List QuaTrinhLamViec cho don
if(donOnline.getQuaTrinhLamViecPTTND() == null || donOnline.getQuaTrinhLamViecPTTND().size() == 0) {
	donOnline.getQuaTrinhLamViecPTTND().add(new DonDTTPHS83.QuaTrinhLamViecPTTND());
}

for(int i = 0; i < donOnline.getQuaTrinhLamViecPTTND().size(); i++) {
	DonDTTPHS83.QuaTrinhLamViecPTTND quaTrinhLamViec = donOnline.getQuaTrinhLamViecPTTND().get(i);
...
List<DmDataItem> listDieuKien = DmDataItemLocalServiceUtil.findByGroupCode(DTConstants.DANH_MUC.DM_CCCM);
...
<aui:form action="<%= actionXemTruocNoiDung %>" name="fm" method="post" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveDon();" %>'>
	<div class="P10 col-xs-12">
		<div class="BGT noidung">
			<div class="col-xs-12">
				<h3 class="tle text-center">ĐƠN ĐỀ NGHỊ<br/>CẤP, CẤP LẠI, CHUYỂN ĐỔI CCCM</h3>
			</div>
			<div class="col-xs-12">
