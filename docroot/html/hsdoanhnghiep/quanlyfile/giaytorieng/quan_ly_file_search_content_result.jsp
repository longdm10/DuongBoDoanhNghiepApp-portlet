...
<%
	System.out.println("------------------vao--quan_ly_file_search_content_result.jsp------------");
	...
	renderHoSoChoTiepNhan.setParameter("jspPage", "/html/hsdoanhnghiep/quanlyfile/giaytorieng/quan_ly_file_search_content.jsp");
	...
	if (userLoginId > 0) {
		...
		resultsQuanLyFile = QuanLyFileLocalServiceUtil.searchQuanLyFile(doanhNghiepId, 2, selTrangThai, tenGiayTo,FormatData.parseStringToDate2(dateNgayCapTuNgay),FormatData.parseStringToDate2(dateNgayCapDenNgay),FormatData.parseStringToDate2(dateNgayHetHanTuNgay),FormatData.parseStringToDate2(dateNgayHetHanDenNgay),FormatData.parseStringToDate2(dateNgayTaiTuNgay),FormatData.parseStringToDate2(dateNgayTaiDenNgay),searchContainer.getStart(), searchContainer.getEnd());
		
		
		longTotal = QuanLyFileLocalServiceUtil.countQuanLyFile(doanhNghiepId, 2, selTrangThai, tenGiayTo, FormatData.parseStringToDate2(dateNgayCapTuNgay),FormatData.parseStringToDate2(dateNgayCapDenNgay),FormatData.parseStringToDate2(dateNgayHetHanTuNgay),FormatData.parseStringToDate2(dateNgayHetHanDenNgay),FormatData.parseStringToDate2(dateNgayTaiTuNgay),FormatData.parseStringToDate2(dateNgayTaiDenNgay));
		...
		<!-- noi dung content -->
				...
				for (QuanLyFile item : resultsQuanLyFile) {
					...
					if(item.getTrangThai().equalsIgnoreCase(Constants.QUAN_LY_FILE_TRANG_THAI_DANG_SU_DUNG)){
						tenTrangThai = YTeConfigurationManager.getStrProp("giay.to.rieng.trang.thai.dang.su.dung", "");
						
						
						
					} else if(item.getTrangThai().equalsIgnoreCase(Constants.QUAN_LY_FILE_TRANG_THAI_KHONG_SU_DUNG)){
						tenTrangThai = YTeConfigurationManager.getStrProp("giay.to.rieng.trang.thai.khong.su.dung", "");
					} 
				...
				<tr>
					...
					<td style="text-align: center;">
						<a href="<%=taiTaiLieuUrl.toString()%>">Xem giấy tờ</a>
					</td>
					...
					<%
						if(item.getTrangThai().equalsIgnoreCase(Constants.QUAN_LY_FILE_TRANG_THAI_DANG_SU_DUNG)){
							...
							%>
							<td style="text-align: center;">
								<p><img class="tabimg2" src="/yte-home-theme/images/i13.png" title="Không chọn được tài liệu"></p>
							</td>
							<%
